...
export default function AIAssistant() {
  ...
  return (
    <>
      ...
      {/* Chat panel */}
      {isOpen && (
        <div className="fixed bottom-24 right-6 z-[90] w-[calc(100vw-3rem)] max-w-sm flex flex-col bg-white rounded-3xl shadow-2xl border border-[#cb6ce6]/40 overflow-hidden" style={{ maxHeight: "70vh" }}>
          {/* Header */}
          ...
          {/* Input */}
          <div className="p-3 border-t border-[#cb6ce6]/30 flex gap-2">
            <input
              type="text"
              value={input}
              ...
              disabled={loading}
              className="flex-1 px-4 py-2.5 bg-zinc-50 border border-[#cb6ce6]/40 rounded-full text-sm focus:outline-none focus:border-primary disabled:opacity-50"
            />
            <button
              onClick={sendMessage}
